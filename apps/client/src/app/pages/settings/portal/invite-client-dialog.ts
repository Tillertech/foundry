...
@Component({
  ...
  host: { class: 'block' },
  imports: [NgIcon, HlmDialogImports, HlmButton, HlmInput, HlmSelectImports, HlmCheckboxImports, Field],
  providers: [provideIcons({ lucideMailCheck, lucideSend })],
  template: `
    <div hlmDialogHeader>
      <h3 hlmDialogTitle>Invite to client portal</h3>
      <p hlmDialogDescription>
        Give {{ ctx.clientName }} secure access to shared projects, documents, quotes and invoices.
      </p>
    </div>

    @if (sent()) {
      <div class="flex flex-col items-center gap-3 py-8 text-center">
        <div class="grid h-12 w-12 place-items-center rounded-full bg-success/15 text-success">
        ...
        <button hlmBtn size="sm" variant="outline" type="button" (click)="dialogRef.close()">Done</button>
      </div>
    } @else {
      <div class="space-y-4 pt-1">
        <div class="grid grid-cols-2 gap-3">
          <app-field label="Full name">
            <input hlmInput class="w-full" [value]="name()" (input)="name.set($any($event.target).value)" placeholder="Jane Doe" />
          </app-field>
          <app-field label="Role">
            <hlm-select class="w-full" [value]="role()" (valueChange)="role.set($event ?? roles[0])">
        ...
        <app-field label="Projects to share" [hint]="projectsHint()">
          <div class="space-y-1.5 rounded-lg border border-border p-2.5">
            @if (clientProjects.length === 0) {
              <p class="px-1 py-2 text-xs text-muted-foreground">This client has no projects yet.</p>
            }
        ...
        <p class="rounded-lg bg-muted/60 px-3 py-2 text-[11px] leading-relaxed text-muted-foreground">
          Portal permissions inherit your workspace defaults. You can override them per project in Settings → Client Portal.
        </p>

        <div hlmDialogFooter>
          <button hlmBtn variant="ghost" size="sm" type="button" (click)="dialogRef.close()">Cancel</button>
          <button hlmBtn size="sm" class="gap-1.5 shadow-[var(--shadow-glow)]" type="button" (click)="submit()">
...
export class InviteClientDialog {
  ...
  protected readonly emailHint = computed(() =>
    this.error() ? undefined : "They'll set their own password from the invite link.",
  );
  protected readonly projectsHint = computed(() =>
    this.selected().length === 0 ? "No selection shares all of this client's projects." : undefined,
  );
  ...
  protected toggleProject(id: string, checked: boolean): void {
    this.selected.update((cur) => (checked ? [...cur, id] : cur.filter((x) => x !== id)));
  }
  ...
  protected submit(): void {
    ...
    this.store.inviteUser({
      ...
      email,
      role: this.role(),
      projectIds: this.selected().length ? this.selected() : this.clientProjects.map((p) => p.id),
    });
    this.sent.set(true);
  }
}
